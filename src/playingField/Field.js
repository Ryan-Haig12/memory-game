...
const Field = ({ gifData, searching }) => {
    ...
    useEffect(() => {
        ...
        if (guesses.secondGuess?.length) {
            ...
            const match_2 =
                guesses.thirdGuess === guesses.secondGuess ||
                guesses.firstGuess === guesses.thirdGuess;
            ...
            if (
                match_2 &&
                !guesses.correctGuesses.includes(guesses.thirdGuess)
            ) {
                let data = guesses.correctGuesses;
    ...
    const getRows = () => {
        ...
        for (let i = 0; i < 4; i++) {
            res.push(
                <Row key={i} style={{ margin: "5px", padding: "5px" }}>
                    {getCols(i)}
                </Row>
            );
        }
    ...
    if (searching)
        return (
            <Spinner
                style={{ position: "relative", left: "50%", marginTop: "10%" }}
                animation="border"
                role="status"
            />
        );
    if (gifData === undefined || randGifs === undefined)
        return <EmptySearchMessage />;
    if (randGifs !== undefined && randGifs.length < 8)
            ...
            {guesses.correctGuesses.length === 8 && (
                <Modal.Dialog style={{ color: "#ededed", background: "black" }}>
                    ...
                    >
                        ...
                        <p>
                            Make another search in the input bar above to play
                            again
                        </p>
                    </Modal.Body>
                </Modal.Dialog>
